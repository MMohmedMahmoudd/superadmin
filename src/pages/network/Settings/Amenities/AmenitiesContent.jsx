...
const AmenitiesContent = () => {
  ...
  const handleRowClick = (amenityId) => {
    navigate(`/editamenity/${amenityId}`);
  };

  const handleViewAmenity = (amenityId) => {
  ...
  const fetchAmenities = async ({ pageIndex, pageSize, sorting }) => {
    try {
      ...
      const params = [
        `perPage=${pageSize}`,
        `page=${pageIndex + 1}`,
        `filter[sp_type_uid]=${selectedCategory}`,
      ];
      ...
      const total = res.data?.pagination?.total ?? amenities.length;
