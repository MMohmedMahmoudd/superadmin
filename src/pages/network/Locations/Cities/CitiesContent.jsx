      ...
      cell: ({ row }) => {
        ...
        const handleToggle = async () => {
          try {
            ...
            if (!token) {
              enqueueSnackbar('Authentication required. Please login again.', { variant: 'error' });
              return;
            }
            ...
            if (image) {
              try {
                ...
                const imageFile = new File([imageBlob], 'city_image.jpg', { type: 'image/jpeg' });
            ...
            await axios.post(`${import.meta.env.VITE_APP_API_URL}/city/${id}/update`, data, {
              headers: {
            ...
            const mainMessage = error?.response?.data?.message || 'Failed to update status';
