...
const CountrySelect = ({ formik }) => {
  ...
  const handleChange = (selectedOption) => {
    formik.setFieldValue('country_uid', selectedOption ? selectedOption.value : '');
    formik.setFieldValue('city_uid', ''); // 🔥 Reset city selection
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <Select
        ...
        isLoading={loading}
        options={options}
        value={options.find(opt => opt.value === +formik.values.country_uid) || null}
        onChange={handleChange}
        placeholder="Select Country"
        styles={customStyles}
      />
      {formik.touched.country_uid && formik.errors.country_uid && (
        <p className="text-red-500 text-xs mt-1">{formik.errors.country_uid}</p>
      )}
    </div>
  );
};
