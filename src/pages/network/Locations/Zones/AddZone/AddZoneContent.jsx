...
const AddZoneContent = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
        // Step 1
        zone_name_en: Yup.string().required('City name in English is required'),
        zone_name_ar: Yup.string().required('City name in Arabic is required'),
        city_uid: Yup.string().required('Country is required'),
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_APP_API_URL}/zone/create`, data, {
        headers: {
