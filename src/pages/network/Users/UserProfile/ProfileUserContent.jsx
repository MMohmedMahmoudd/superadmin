...
const ProfileUserContentComponent = (
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        if (
          values.person_status !== undefined &&
          values.person_status !== null
        ) {
          const statusVal =
            statusMap[String(values.person_status).toLowerCase()] ??
            values.person_status;
        ...
        const errorMessage =
          error?.response?.data?.errors?.person_email?.[0] ||
          "Failed to update user.";
        const msg = errorMessage || "Failed to update user.";
        enqueueSnackbar(msg, { variant: "error" });
        enqueueSnackbar(error?.response?.data?.person_email, {
          variant: "error",
        });
      } finally {
        setSubmitting(false);
      ...
      <div className="mt-1">
        ...
        >
          ...
          <TabPanel value="Reservations">
            <div>
              <ReservationTab personUid={providerId} />
            </div>
          </TabPanel>
        </Tabs>
      </div>
