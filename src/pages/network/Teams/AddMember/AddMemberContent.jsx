...
const AddMemberContent = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_APP_API_URL}/team/create`, data, {
        headers: {
      ...
      if (responseData) {
        ...
        if (responseData.data && typeof responseData.data === 'object') {
          ...
          console.log('Field errors from catch block:', fieldErrors);
          enqueueSnackbar(fieldErrors, { variant: 'error' });
          Object.entries(fieldErrors).forEach(([field, messages]) => {
            if (Array.isArray(messages) && messages.length > 0) {
              console.log(`Setting error for field ${field} in catch:`, messages[0]);
              enqueueSnackbar(messages[0], { variant: 'error' });
              formik.setFieldError(field, messages[0]);
              formik.setFieldTouched(field, true);
            }
...
<form className="w-full" onSubmit={formik.handleSubmit}>
  ...
  <div  className="grid grid-cols-1 xl:grid-cols-3 gap-4">
    ...
    <div className=" flex justify-center items-center dark:bg-gray-200 bg-gray-100 rounded-lg py-4 flex-col gap-y-4 mb-4">
      <label className="block text-sm font-medium mb-1">Member Logo  </label> 
      <CrudAvatarUpload onFileChange={handleFileChange} />
      {formik.errors.person_image && <p className="text-red-500 text-sm mt-1">{formik.errors.person_image}</p>}
  ...
  {formik.touched.person_status && formik.errors.person_status && (
    ...
    <div className="col-span-3 xl:col-span-2 card p-6">
      <h3 className="text-lg font-semibold mb-4 text-gray-900">Member Information</h3>
