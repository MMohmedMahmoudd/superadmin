...
const MemberProfileContent = forwardRef(({ memberId, member, profileImage, setFormikMeta }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      person_password: Yup.string().min(6, 'Password must be at least 6 characters'),
          ...
          if (responseData.data && typeof responseData.data === 'object') {
            ...
            console.log('Field errors from API:', fieldErrors);
             ...
             if (Object.keys(fieldErrors).length > 0) {
               enqueueSnackbar('Please fix the validation errors below.', { variant: 'error' });
             }
          }
  ...
  useEffect(() => {
    ...
    formik.setFieldValue('person_image', profileImage || memberData?.person?.person_image || null);
  
    // Show snackbar error if nothing exists AND user tried to submit
    if (!profileImage && !memberData?.person?.person_image && formik.submitCount > 0) {
...
<Tabs value={activeTab} onChange={(_, newValue) => setActiveTab(newValue)}>
        <TabsList className="flex flex-wrap">
          ...
          {memberData?.providers?.length > 0 && <Tab value="Business">Business</Tab>}
