        ...
        if (values.selectedOptions && values.selectedOptions.length > 0) {
          values.selectedOptions.forEach((option, index) => {
            if (option.options_id && option.qty && parseInt(option.qty) > 0) {
              ...
              data.append(`booking_options[${index}][option_id]`, option.options_id);
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          ...
          <div className="col-span-3 xl:col-span-2 card p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {/* Options Selection */}
              {formik.values.offerData?.options && formik.values.offerData.options.length > 0 && (
                <div className="col-span-2">
                  <label className="form-label mb-2">Select Options</label>
                  <div className="border rounded-lg p-4 space-y-3">
                    {formik.values.offerData.options.map((option) => {
                      const selectedOption = formik.values.selectedOptions.find(
                      ...
                      return (
                        <div key={option.id} className="flex items-center gap-4 p-3 border rounded-md">
                          <input
                            ...
                            checked={isSelected}
                            onChange={(e) => {
                              const subset = formik.values.selectedOptions.filter(
                                (opt) => opt.options_id !== option.id.toString()
                              );
                              if (e.target.checked) {
                                ...
                                formik.setFieldValue('selectedOptions', subset);
                              }
                            }}
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-sm text-gray-500">Price: {option.price} {formik.values.offerData?.currency || formik.values.offerData?.currency_name || 'EGP'}</div>
                            ...
                            <div className="w-32">
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  const updatedOptions = formik.values.selectedOptions.map((opt) =>
                                    opt.options_id === option.id.toString()
                                      ? { ...opt, qty: value }
                                      : opt
                                  );
                                  formik.setFieldValue('selectedOptions', updatedOptions);
                                }}
                                onBlur={() => {
                                  // Ensure qty is at least 1 if option is selected
                                  const currentOption = formik.values.selectedOptions.find(
                                    (opt) => opt.options_id === option.id.toString()
                                  );
                  ...
                  {formik.values.selectedOptions.length > 0 && formik.values.selectedOptions.some(opt => !opt.qty || parseInt(opt.qty) < 1) && (
                    <p className="text-red-500 text-xs mt-1">Please enter a valid quantity for all selected options</p>
                  )}
                </div>
              )}
              ...
              <div className="col-span-1">
                ...
                {formik.touched.arrival_date && formik.errors.arrival_date && (
                  <p className="text-red-500 text-xs mt-1">{formik.errors.arrival_date}</p>
                )}
              </div>
