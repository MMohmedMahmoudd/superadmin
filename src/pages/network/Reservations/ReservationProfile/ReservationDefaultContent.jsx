...
const ReservationDefaultContent = () => {
        ...
        if (res.data.data.offer_uid) {
          try {
            ...
            if (offerRes.data?.data) {
              ...
              if (res.data.data.booking_options && Array.isArray(res.data.data.booking_options)) {
                setSelectedOptions(
                  res.data.data.booking_options
                    ...
                    .map((opt) => ({
                      ...
                      qty: opt.qty != null && opt.qty > 0 ? String(opt.qty) : '1',
                    }))
                );
              } else {
                setSelectedOptions([]);
  ...
  const handleAddCoupons = async () => {
    try {
      ...
      if (validCouponCodes.length === 0) {
        enqueueSnackbar('Please enter at least one coupon code', { variant: 'error' });
        return;
      }
      ...
      enqueueSnackbar(
        error.response?.data?.message || 'An error occurred while adding coupons.',
        { variant: 'error' }
      );
    }
  };
  ...
  const handleDateChange = async (customStatus) => {
    try {
      ...
      if (!personUid) {
        enqueueSnackbar('Invalid reservation data: missing person information', { variant: 'error' });
        return;
      }
      ...
      const payload = {
        ...
        booking_status: customStatus ?? parseInt(bookingStatus || reservationData.status_num, 10),
        ...
        payment_obj: {
          ...
          payment_amount: reservationData.offer_price * reservationData.num_coupons,
      ...
      const optionsFromReservation = Array.isArray(reservationData.booking_options)
      ...
      const optionsToSend = (selectedOptions && selectedOptions.length > 0)
        ? selectedOptions
      ...
      if (optionsToSend && optionsToSend.length > 0) {
        ...
        if (invalidOptions.length > 0) {
          enqueueSnackbar('Please enter a valid quantity for all selected options', { variant: 'error' });
          return;
        }

        optionsToSend.forEach((option, index) => {
          formData.append(`booking_options[${index}][option_id]`, option.options_id);
      ...
      const hasError = res.data?.data?.error || 
      (res.data?.message && (
      ...
      if (res.data?.success && !hasError) {
        enqueueSnackbar(res.data?.message || 'Arrival date updated successfully!', { variant: 'success' });
        setIsDateModalOpen(false);
        fetchReservation(); // Refresh reservation data
      } else {
        // Show error message from data.error if available, otherwise use message
        const errorMsg = res.data?.data?.error || 
                        res.data?.message || 
      ...
      const userErrorMessage = error.response?.data?.message 
  ...
  const currentStatus = normalizeStatus(reservationData.status || reservationData.status_name);
    ...
    <>
      ...
      <div className="  card shadow-sm">
        ...
        <div className=" card-body  ">
          ...
          <img
              src={reservationData.person_image || toAbsoluteUrl('/media/avatars/blank.png')}
            ...
            <div className="flex flex-col">
              ...
              <span className="text-md flex justify-center items-center gap-2 text-gray-500">
                <KeenIcon icon="phone" className=' h-4' aria-hidden="true"/> <span className='flex justify-center items-center '>{reservationData.person_phone}</span>
                <KeenIcon icon="sms" className=' h-4' aria-hidden="true"/> <span className='flex justify-center items-center '>{reservationData.person_email}</span>
              </span>
            </div>
          </div>  
      ...
      <div className="card shadow-sm">
        ...
        <div className="card-body">
          <div className="card p-4">
            ...
            <h3 className='card-title'>{reservationData.booking_date ? new Date(reservationData.booking_date).toLocaleDateString('en-GB', {
              day: '2-digit',
              month: 'short',
              year: 'numeric',
            }) : '-'}</h3>
            ...
            <button 
              ...
              aria-label={canEditArrivalDate ? 'Edit Arrival Date' : 'Arrival Date Confirmed'}
            > 
        ...
        <div className="card shadow-sm ">
          ...
          <div className="card-body ">
            <div className="flex w-full items-center justify-between gap-2 mt-2">
              <label htmlFor="booking-status-select" className="text-md">Status</label>
              <select
      ...
      {offerData?.options && offerData.options.length > 0 && selectedOptions.length > 0 && (
          ...
          <div className="card-body">
            <div className="space-y-3">
              {selectedOptions.map((selectedOpt) => {
                ...
                return (
                  <div key={selectedOpt.options_id} className="flex items-center gap-4 p-3 border rounded-md">
                    <div className="flex-1">
                      <div className="font-semibold">{optionDetails.name}</div>
                      {optionDetails.description && (
                        <div className="text-sm text-gray-600">{optionDetails.description}</div>
                      )}
                      <div className="text-sm text-gray-500">
                        Price: {optionDetails.price} {offerData?.currency || offerData?.currency_name || 'EGP'} x {selectedOpt.qty}
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <div className="text-sm text-gray-500 mt-1">
                        Total: {parseInt(optionDetails.price) * parseInt(selectedOpt.qty || 0)} {offerData?.currency || offerData?.currency_name || 'EGP'}
                      </div>
                    </div>
                  </div>
                );
                ...
                <div className="mt-4 border-t pt-4">
                  ...
                  <div className="border rounded-lg p-4 space-y-3">
                    {offerData.options.map((option) => {
                      ...
                      return (
                        <div key={option.id} className="flex items-center gap-4 p-3 border rounded-md">
                          <input
                            type="checkbox"
                            checked={isSelected}
                            onChange={(e) => {
                              ...
                              if (e.target.checked) {
                                setSelectedOptions([
                                  ...subset,
                                  { options_id: option.id.toString(), qty: '1' },
                                ]);
                              } else {
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-sm text-gray-500">Price: {option.price} {offerData?.currency || offerData?.currency_name || 'EGP'}</div>
                            <div className="text-sm text-gray-500">Available: {option.qty}</div>
                          </div>
                          {isSelected && (
                            <div className="w-32">
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  const sanitizedValue = value === '' ? '' : (numValue > 0 ? String(numValue) : '1');
                                ...
                                onBlur={() => {
                                  ...
                                  if (currentOption && (!currentOption.qty || parseInt(currentOption.qty) < 1)) {
                                    const updatedOptions = selectedOptions.map((opt) =>
                                      opt.options_id === option.id.toString()
                                        ? { ...opt, qty: '1' }
                                        : opt
        ...
        <div className="card-header">
          ...
          <h3 className='card-title'>
            {(() => {
              ...
              const allCoupons = reservationData.booking_options?.flatMap(bo => bo.coupons || []) || [];
              const usedCount = allCoupons.filter(coupon => coupon.is_used === "1" || coupon.is_used === 1).length;
          ...
          <div className="flex flex-col gap-2">
            <div className="grid grid-cols-1 gap-2">
                  ...
                  allCoupons.map((coupon, index) => (
                    <div key={index} className="flex justify-between w-full items-center">
                      ...
                      {(coupon.is_used === "1" || coupon.is_used === 1) ? (
                        <KeenIcon icon='check-circle' className='text-success text-xl' aria-label="Coupon used" />
                      ) : null}
                    </div>
                  ))
                ) : (
              ...
              {(() => {
                // Flatten all coupons from all booking_options
                const allCoupons = reservationData.booking_options?.flatMap(bo => bo.coupons || []) || [];
                const hasUnusedCoupons = allCoupons.some(coupon => coupon.is_used !== "1" && coupon.is_used !== 1);
  ...
  <button
    ...
    type="button"
    onClick={() => handleDateChange(parseInt(bookingStatus || reservationData.status_num, 10))}
  >
    Update Reservation
  </button>
</div>
      ...
      <Modal open={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <div className="card  w-1/2 h-auto justify-center gap-4 z-[9999]" style={{ margin: 'auto', maxWidth: '500px' }}>
          <div className="card-header">
            <h2 className="card-title">Add Coupon</h2>
            <button onClick={() => setIsModalOpen(false)} aria-label="Close add coupon modal"><KeenIcon icon='cross-circle' className='text-xl' aria-hidden="true"/> </button>
          </div>
          <div className="card-body ">
              ...
              <div key={index} className={`form-control ${index > 0 ? 'mt-4' : ''}`}>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    const sanitizedValue = e.target.value.trim().replace(/[<>"'&]/g, '');
      ...
      <Modal open={isDateModalOpen} onClose={() => setIsDateModalOpen(false) }>
        <div className="card w-1/2 h-1/2 items-center justify-center gap-4 relative " style={{ margin: 'auto', maxWidth: '500px' }}>
          <div className="card-header w-full">
            <h2 className="card-title">Edit Arrival Date</h2>
            <button onClick={() => setIsDateModalOpen(false)} aria-label="Close edit arrival date modal"><KeenIcon icon='cross-circle' className='text-xl' aria-hidden="true"/></button>
          </div>
          ...
          <div className="card-footer justify-end w-full gap-4">
            <button className="btn border-1 border-gray-100 bg-transparent btn-secondary hover:border-0" onClick={() => setIsDateModalOpen(false)}>Cancel</button>
            <button className="btn btn-outline btn-primary" onClick={() => handleDateChange(6)}>Update Date</button>
          </div>
        </div>
      </Modal>
    </>
