...
const AmenitiesSidebar = ({ open, onClose, spTypeUid, onAmenitySelect, editingOption }) => {
    ...
    if (editingOption) {
      ...
      const images = Array.isArray(editingOption.images) 
        ...
        : editingOption.image 
          ? [editingOption.image] 
          : [];
      const normalizedAmenities = (editingOption.amenities || []).map(normalizeAmenity);
      setOfferOptionForm({
    ...
    setOfferOptionForm(prev => {
      ...
      prev.images.forEach(img => {
        if (img && img.startsWith('blob:')) {
          try { URL.revokeObjectURL(img); } catch { /* noop */ }
        }
      });
  ...
  const handleCreateOfferOption = async () => {
    if (!offerOptionForm.price || !offerOptionForm.couponQuantity || !offerOptionForm.titleEnglish || !offerOptionForm.titleArabic) {
    ...
    enqueueSnackbar(`Offer option ${action} successfully!`, { variant: 'success' });
    onClose();
  };
  ...
  const writePreviewsToLS = (records) => {
    try { localStorage.setItem(LS_KEY, JSON.stringify(records)); } catch { /* noop */ }
  };
    ...
    <>
      {open && (
        <>
          ...
          {/* Sidebar */}
          <div 
            className="fixed card pb-4 right-0 top-0 h-full w-[600px] max-w-[90vw] bg-gray-100 dark:bg-gray-100 shadow-2xl z-50 overflow-hidden overflow-y-auto"
          >
              {/* Header - Fixed */}
              <div className="flex items-center justify-between mb-6 flex-1 overflow-y-auto max-h-[100px] card-header">
              ...
              <div className="card mb-4 dark:bg-gray-200 bg-gray-100 rounded-lg">
                ...
                maxFiles={8}
              />
              <p className="text-sm text-center text-gray-500 mt-1">Only *.png, *.jpg, and *.jpeg image files are accepted. Drag images to reorder.</p>
              ...
              <div className="mb-6 p-4 border rounded-lg bg-gray-200 ">
                ...
                <div className="mb-4">
                  <label className="label">
                    <span className="label-text">Offer Description (English)</span>
                  </label>
                  <textarea
              ...
              <div className="flex-1 ">
                <div className="mb-4">
                  <h3 className="text-lg font-medium mb-2">Do you have any amenities?</h3>
                </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex flex-col items-center text-center">
                            <img 
                              src={amenity.icon || toAbsoluteUrl('/media/avatars/blank.png')} 
                              alt={amenity.name}
                            ...
                            {amenity.name_arabic && (
                              <div className="text-xs text-gray-500 mt-1">{amenity.name_arabic}</div>
                            )}
                          </div>
                        </div>
                      );
              ...
              <div className="mt-6 pt-4 border-t flex-shrink-0 card-footer">
                <div className="flex w-full justify-between items-center">
                  ...
                  >
                    {editingOption ? 'Update Offer Option' : 'Create Offer Option'}
                  </button>
                </div>
              </div>
            </div>
