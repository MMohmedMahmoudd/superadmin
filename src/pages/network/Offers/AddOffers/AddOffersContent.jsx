...
const AddOffersContent = () => {
  ...
  useEffect(() => {
    try {
      ...
      setOfferOptions(prev => prev.map(opt => {
        if ((!opt.images || opt.images.length === 0) && (!opt.image) && opt.id) {
...
// Before sending form
    ...
    data.append('branches_id', JSON.stringify(branchesPayload));
      ...
      try {
        ...
        for (const [key, value] of data.entries()) {
          if (value instanceof File) {
            debugEntries.push({ key, value: `File(name=${value.name}, size=${value.size})` });
          } else {
      ...
      if (errorMessages.length > 0) {
        ...
        enqueueSnackbar(`${mainMessage}: ${errorMessages[0]}`, { variant: 'error' });
        // Log all errors to console
        console.group('⚠️ Validation Errors:');
        errorMessages.forEach(msg => console.error(msg));
        console.groupEnd();
      } else {
        enqueueSnackbar(mainMessage || 'Something went wrong. Please try again.', { variant: 'error' });
      }
    ...
    {/* Multi Language Content Card */}
      ...
      <TabsList className="w-full mb-4">
          ...
          <TabPanel value="Arabic" className="grid  gap-4">
            <div className="form-group">
            <label className="form-label mb-1" dir="rtl">إسم العرض</label>
              <input type="text" className="input" dir="rtl" {...formik.getFieldProps('offer_title_arabic')} placeholder=" إسم العرض..." />
            ...
            <div className="form-group">
              <label className="form-label mb-1" dir="rtl">وصف العرض</label>
              <RichTextEditor
                  onBlur={(name) => formik.setFieldTouched(name, true)}
                  className="rtl"
            ...
            <div className="form-group">
              <label className="form-label mb-1" dir="rtl">سياسة الإلغاء</label>
              <RichTextEditor
                  onBlur={(name) => formik.setFieldTouched(name, true)}
                  className="rtl"
            ...
            <div className="form-group">
            <label className="form-label mb-1" dir="rtl">الشروط والأحكام</label>
              <RichTextEditor
                  onBlur={(name) => formik.setFieldTouched(name, true)}
                  className="rtl"
