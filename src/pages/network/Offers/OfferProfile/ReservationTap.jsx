...
const ReservationTap = ({ offerId }) => {
  ...
  const fetchReservations = async ({ pageIndex, pageSize, sorting }) => {
    try {
      ...
      const responseCurrency = res.data?.currency || res.data?.currency_name || 'EGP';
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const rowCurrency = row.original.currency_name || 
        ...
        return (
          <span className="text-sm font-medium">
            {offer_price && rowCurrency ? `${offer_price} ${rowCurrency}` : offer_price || '-'}
          </span>
        );
      },
      enableSorting: true,
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span className="text-sm">{formatDate(row.original.booking_date)}</span>
      ),
      enableSorting: true,
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const firstCoupon = row.original.coupons && row.original.coupons.length > 0 
          ? row.original.coupons[0] 
          : null;
        return (
          <span className="text-sm">
            {firstCoupon?.expire_date ? formatDate(firstCoupon.expire_date) : '-'}
          </span>
        );
      },
      enableSorting: true,
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <span className={`badge badge-${badgeColor} badge-outline capitalize`}>
            ● {statusName}
          </span>
        );
      },
