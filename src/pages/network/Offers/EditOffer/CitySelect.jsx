...
const CitySelect = ({ formik }) => {
    ...
    const fetchCities = async () => {
      try {
        ...
        const formatted = cities.map(city => ({
          value: city.id,
          label: normalizeLabel(city.name, 'Unnamed City'),
        }));

        formatted.unshift({ value: '', label: 'All Cities' });
  ...
  const handleChange = (selectedOption) => {
    formik.setFieldValue('city_uid', selectedOption?.value || '');
    formik.setFieldValue('city_name', selectedOption?.label || '');
    formik.setFieldValue('zone_uid', '');
    formik.setFieldValue('zone_name', '');
    };
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <Select
        ...
        onBlur={() => {
          formik.setFieldTouched('city_uid', true);
        }}
