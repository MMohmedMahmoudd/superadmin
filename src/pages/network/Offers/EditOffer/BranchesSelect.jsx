...
const BranchesSelect = ({ offer, selectedCity, formik, providerId }) => {
  ...
  const filteredBranches = useMemo(() => {
    ...
    const normalizedCity = typeof selectedCity === 'string' 
      ? selectedCity 
    ...
    return branches.filter(branch => branch.cityName?.toLowerCase() === normalizedCity.toLowerCase());
  }, [branches, selectedCity]);
  
  const selectedBranches = useMemo(() => {
    if (!formik.values.branches_id || !Array.isArray(formik.values.branches_id)) {
      return [];
    }
    const selected = filteredBranches.filter(opt => {
      const optValue = opt.value.toString();
      const isSelected = formik.values.branches_id.some(id => id.toString() === optValue);
      return isSelected;
    });
