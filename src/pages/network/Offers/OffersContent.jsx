...
const OffersContent = () => {
  ...
  useEffect(() => {
    setRefetchKey(prev => prev + 1);
  }, [debouncedSearch, statusFilter, fromDate, toDate, bestOfferFilter, thingsToDoFilter]);
  ...
  const columns = useMemo(() => [
        ...
        // If offer_images is an array, display the first image
          return<> 
            <div className="flex items-center justify-start gap-x-4">
              <img 
                src={business_image || toAbsoluteUrl('/media/avatars/blank.png')} 
                alt={city} 
...

    {
      ...
      cell: ({ row }) => {
        return <span className="text-sm ">{row.original.offer_copouns_qty}</span>
      },
    ...
    },
    {
      ...
      enableSorting: true,
      cell: ({ row }) => {
        const thingsToDo = row.original.options?.[0]?.offer?.offer_things_to_do;
