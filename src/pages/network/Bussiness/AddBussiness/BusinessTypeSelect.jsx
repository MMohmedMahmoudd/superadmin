...
const BusinessTypeSelect = ({ formik }) => {
  ...
  const handleChange = (selectedOption) => {
    formik.setFieldValue('sp_type_uid', selectedOption ? selectedOption.value : '');
  };
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <Select
        ...
        isLoading={loading}
        options={options}
        value={options.find(opt => opt.value === +formik.values.sp_type_uid) || null}
        onChange={handleChange}
        placeholder="Select Business Type"
        styles={customStyles}      />
      {formik.touched.sp_type_uid && formik.errors.sp_type_uid && (
        <p className="text-red-500 text-xs mt-1">{formik.errors.sp_type_uid}</p>
      )}
    </div>
  );
};

export { BusinessTypeSelect };
