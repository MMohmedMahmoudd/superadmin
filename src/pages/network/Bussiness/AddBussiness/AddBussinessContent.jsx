...
const AddBussinessContent = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      branch_email: Yup.string().email('Invalid email').required('Email is required'),
      branch_phone: Yup.string()
      .required('Mobile number is required')
      .matches(/^\d+$/, 'Must be numeric only'), // ensures it's numbers only
    
      sp_name_english: Yup.string().required('Business name in English is required'),
      sp_name_arabic: Yup.string().required('Business name in Arabic is required'),
      sp_description_english: Yup.string().required('Business description in English is required'),
      sp_description_arabic: Yup.string().required('Business description in Arabic is required'),
        ...
        onSubmit: async (values) => {
          try {
            ...
            const token = JSON.parse(localStorage.getItem(
             ...
             await axios.post(`${import.meta.env.VITE_APP_API_URL}/providers/create`, data, {
              headers: {
...
<form className="w-full" onSubmit={formik.handleSubmit}>
  ...
  <select
    className="select"
    {...formik.getFieldProps('sp_status')}
  >
  ...
  {formik.touched.sp_status && formik.errors.sp_status && (
    <span role="alert" className="text-danger text-xs mt-1">{formik.errors.sp_status}</span>
  )}
</div>
<div className="  mt-5 card" >
      <RecentUploads title="Documents" setFieldValue={formik.setFieldValue}/>
      </div>
    ...
    <div className="col-span-3 xl:col-span-2 card p-6">
      ...
      <div>
        ...
        <div className='col-span-2'>
          ...
          />
          {formik.touched.sp_name_english && formik.errors.sp_name_english && (
            <p className="text-red-500 text-xs mt-1">{formik.errors.sp_name_english}</p>
          )}
        </div>
        <div className='col-span-2'>
          ...
          />
          {formik.touched.sp_name_arabic && formik.errors.sp_name_arabic && (
            <p className="text-red-500 text-xs mt-1">{formik.errors.sp_name_arabic}</p>
          )}
        </div>
        <div className="col-span-2">
          ...
          />
          {formik.touched.sp_description_english && formik.errors.sp_description_english && (
            <p className="text-red-500 text-xs mt-1">{formik.errors.sp_description_english}</p>
          )}
        </div>
        <div className="col-span-2">
          ...
          />
          {formik.touched.sp_description_arabic && formik.errors.sp_description_arabic && (
            <p className="text-red-500 text-xs mt-1">{formik.errors.sp_description_arabic}</p>
          )}
        </div>          
      </div>
    </div>
  ...
  <div className=" col-span-3 card p-6 mt-5">
          ...
          <div  className="grid col-span-3 xl:col-span-2 gap-4">
            <div className="col-span-2">
              <h3 className="text-lg font-semibold mb-4 text-gray-900">Main Branch Information</h3>
            </div>
            <div className="col-span-1">
              <label className="form-label mb-1">Branch Name (English)</label>
              <input
                className={clsx('input', {
                  'is-invalid': formik.touched.branch_name_english && formik.errors.branch_name_english
                })}
                {...formik.getFieldProps('branch_name_english')}
              />
              {formik.touched.branch_name_english && formik.errors.branch_name_english && (
                <p className="text-red-500 text-xs mt-1">{formik.errors.branch_name_english}</p>
              )}
          </div>
            <div className="col-span-1">
              ...
              <input
                className={clsx('input', {
                  'is-invalid': formik.touched.branch_name_arabic && formik.errors.branch_name_arabic
                })}
                {...formik.getFieldProps('branch_name_arabic')}
              />
              {formik.touched.branch_name_arabic && formik.errors.branch_name_arabic && (
                <p className="text-red-500 text-xs mt-1">{formik.errors.branch_name_arabic}</p>
              )}
          </div>
          <div>
          <label className="form-label mb-1">Branche Email</label>
          <input
            className="input"
            {...formik.getFieldProps('branch_email')}
          />
          {formik.touched.branch_email && formik.errors.branch_email && (
            <p className="text-red-500 text-xs mt-1">{formik.errors.branch_email}</p>
          )}
        </div>
        <div>
            ...
            <p className="text-red-500 text-xs mt-1">{formik.errors.branch_phone}</p>
          )}
          ...
          <div>
            ...
            <input
              className={clsx('input', {
                'is-invalid': formik.touched.branch_address_english && formik.errors.branch_address_english
              })}
              {...formik.getFieldProps('branch_address_english')}
            />
            {formik.touched.branch_address_english && formik.errors.branch_address_english && (
              <p className="text-red-500 text-xs mt-1">{formik.errors.branch_address_english}</p>
            )}
          </div>
          <div>
            ...
            <input
              className={clsx('input', {
                'is-invalid': formik.touched.branch_address_arabic && formik.errors.branch_address_arabic
              })}
              {...formik.getFieldProps('branch_address_arabic')}
            />
            {formik.touched.branch_address_arabic && formik.errors.branch_address_arabic && (
              <p className="text-red-500 text-xs mt-1">{formik.errors.branch_address_arabic}</p>
            )}
          </div>
          <div>
      ...
      <input
        className={clsx('input', {
          'is-invalid': formik.touched.branch_latitude && formik.errors.branch_latitude
        })}
        {...formik.getFieldProps('branch_latitude')}
        readOnly
      />
      {formik.touched.branch_latitude && formik.errors.branch_latitude && (
        <p className="text-red-500 text-xs mt-1">{formik.errors.branch_latitude}</p>
      )}
    </div>
    <div>
      ...
      <input
        className={clsx('input', {
          'is-invalid': formik.touched.branch_longitude && formik.errors.branch_longitude
        })}
        {...formik.getFieldProps('branch_longitude')}
        readOnly
      />
      {formik.touched.branch_longitude && formik.errors.branch_longitude && (
        <p className="text-red-500 text-xs mt-1">{formik.errors.branch_longitude}</p>
      )}
    </div>
        ...
        />
<LocationSelector setLatLng={({ branch_latitude, branch_longitude }) => {
