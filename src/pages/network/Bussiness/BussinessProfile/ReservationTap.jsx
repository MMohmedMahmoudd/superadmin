...
const ReservationTap = ({ providerId }) => {
  ...
  const fetchReservations = async ({ pageIndex, pageSize, sorting }) => {
    try {
      ...
      const baseUrl = `${import.meta.env.VITE_APP_API_URL}/reservations/list` +
  ...
  const columns = useMemo(() => [
...

    {
      ...
      cell: ({ row }) => (
          <span className={`badge flex-nowrap badge-${row.original.status_name === 'Waiting Payment' ? 'primary' : row.original.status_name === 'confirmed' ? 'success' : row.original.status_name === 'cancelled' ? 'danger' : row.original.status_name === 'expired' ? 'danger' : row.original.status_name === 'completed' ? 'success' : row.original.status_name === 'Active' ? 'success' : 'warning'} badge-outline capitalize`}>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <button
          ...
          onClick={() => navigate(`/reservationprofile/${row.original.booking_uid}`)}
        >
    ...
    <div className="card-body p-0 overflow-x-auto relative">
      <div className="card-header px-2 flex-wrap gap-2 justify-between">
        <h3 className="card-title ">
          Showing {totalCount} Reservations
        </h3>
