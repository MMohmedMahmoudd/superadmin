...
const ProviderBranchesTable = ({ providerId }) => {
  ...
  const fetchProviderData = async ({ pageIndex, pageSize }) => {
    try {
      ...
      const filteredBranches = branchesData.filter(branch => {
        ...
        // Handle city filter
        if (
          formik.values.city_uid && formik.values.city_uid !== '' &&
      ...
      if (
        formik.values.zone_uid && formik.values.zone_uid !== '' &&
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <button
          className="px-2 py-1 text-blue-500 btn btn-sm btn-outline btn-primary"
    ...
    <div className="card-body p-0 overflow-x-auto relative">
      <div className="card-header px-2 flex-wrap gap-2 justify-between">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <button className="btn btn-sm btn-outline btn-primary" onClick={() => navigate(`/addBranche/${providerId}`)}>Add New Branch</button>
        </div>
      </div>
