...
const OffersTable = ({ providerId }) => {
  ...
  const fetchOffers = async ({ pageIndex, pageSize, sorting }) => {
    try {
      ...
      let url = `${import.meta.env.VITE_APP_API_URL}/provider/offers?filter[sp_uid]=${providerId}` +
        `&perPage=${pageSize}&page=${pageIndex + 1}` +
        (sort ? `&sort=${sort}` : '');

        if (debouncedSearch) {
          const isOfferId = /^\d+$/.test(debouncedSearch); // if user typed a number only
          if (isOfferId) {
            url += `&filter[offer_uid]=${debouncedSearch}`;
          } else {
        ...
        return (
          <div className="flex items-center gap-x-2">
            ...
            onError={(e) => {
             ...
             alt={name} className="w-10 h-10 rounded-full object-cover" />
            <span>{name}</span>
          </div>
        );
      },
