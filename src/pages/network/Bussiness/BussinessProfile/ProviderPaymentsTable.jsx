...
const ProviderPaymentsTable = ({ providerId, provider }) => {
  ...
  const fetchPayments = async ({ pageIndex, pageSize, sorting }) => {
    try {
      ...
      const sortParam =
        sort && allowedSorts.includes(sort) ? `${sortOrder}${sort}` : "";

      let filterParam = `&filter[sp_uid]=${providerId}`; // always add the providerId filter first

      if (debouncedSearch) {
        const cleanedSearch = debouncedSearch.trim();
        const formattedTotal = cleanedSearch;

        if (/^\d+(\.\d{1,2})?$/.test(cleanedSearch)) {
          // 1. Try filtering by invoice_uid first
          const tryIdUrl = `${import.meta.env.VITE_APP_API_URL}/provider/payments?perPage=1&page=1&filter[sp_uid]=${providerId}&filter[invoice_uid]=${cleanedSearch}`;
      ...
      const url =
        `${import.meta.env.VITE_APP_API_URL}/provider/payments` +
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-sm font-medium">
            ${row.original.invoice_total.toFixed(2)}
          </span>
        ),
        enableSorting: false,
      },
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return (
            <span
              className={`badge badge-${color} badge-outline capitalize whitespace-nowrap`}
            >
              {label}
            </span>
          );
  ...
  return (
    <div className="card p-6">
      <div className="card-header px-2 flex-wrap gap-2 justify-between">
        <h3 className="card-title">Showing {totalCount} Payments</h3>
            ...
            <div className="mb-4 card ">
              <div className="card-header">
                <h3 className=" card-title ">
                  Payment sent to {selectedRow.provider?.name}
                </h3>
              </div>
              <div className="card-body">
                <div className="grid grid-cols-3 gap-2 text-sm">
                  <div>
                    ...
                    <b>
                      {new Date(
                        selectedRow.invoice_issuance_date
                      ).toLocaleDateString()}
                    </b>
                  </div>
                  <div>
                    Transaction Type: <b>{selectedRow.invoice_type}</b>
                  </div>
                  <div>
                    Gross Amount: <b>${selectedRow.invoice_total}</b>
                  </div>
                  <div>
                    Invoice Id: <b>#{selectedRow.invoice_uid}</b>
                  </div>
                  <div>
                    Status: <b>{selectedRow.invoice_status}</b>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="card-body px-0 py-0">
                <table className="w-full text-sm  border border-gray-200 rounded-lg overflow-hidden">
                  <thead className="bg-gray-100 dark:bg-gray-200 ">
                    <tr>
                      <th className="px-7 py-2 border-b border-gray-200 text-left font-semibold">
                        Invoice ID
                      </th>
                      <th className="px-4 py-2 border-b border-gray-200 text-left font-semibold">
                        Name
                      </th>
                      <th className="px-4 py-2 border-b border-gray-200 text-left font-semibold">
                        Price
                  ...
                  <tbody>
                    <tr className="hover:bg-gray-50 dark:hover:bg-gray-100">
                      <td className="px-7 py-2 border-b border-gray-200">
                        #{selectedRow.invoice_uid}
                      </td>
                      ...
                      <td className="px-4 py-2 border-b border-gray-200">
                        ${selectedRow.invoice_total}
                      </td>
                    </tr>
                  </tbody>
                  <tfoot className="bg-gray-100 dark:bg-gray-200 w-full ">
                    <tr className="w-full">
                      <td className="px-7 w-1/2 py-2 border-b border-gray-200">
                        Net Amount
                      </td>
                      <td className="px-4 w-1/2 py-2 border-b border-gray-200">
                        ${selectedRow.invoice_total}
                      </td>
