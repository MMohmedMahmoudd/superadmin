...
const BussinessDefaultContent = ({ provider }) => {
  ...
  const [branchNameEn, setBranchNameEn] = useState(provider.main_branch?.name_both_lang?.en || "");
  const [branchNameAr, setBranchNameAr] = useState(provider.main_branch?.name_both_lang?.ar || "");
  const [branchEmail, setBranchEmail] = useState(provider.main_branch?.email || "");
  const [nameEn, setNameEn] = useState(provider.name || "");
  const [nameAr, setNameAr] = useState(provider.name || "");
  const [descriptionEn, setDescriptionEn] = useState(provider.description || "");
  const [descriptionAr, setDescriptionAr] = useState(provider.description || "");
  ...
  const handleUpdateProvider = async () => {
    ...
    if (!branchNameEn.trim() || !branchNameAr.trim()) {
      enqueueSnackbar('Branch name in both English and Arabic is required.', { variant: 'error' });
      setLoading(false);
      return;
    }
    if (!branchEmail.trim()) {
              ...
              <div className="card p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-1">
                    ...
                    <input className="input" value={provider.type?.name || ''} disabled />
                  </div>
                  <div className="col-span-1">
                    <label className="form-label mb-1">Percentage</label>
                    <input className="input" value={provider.commission_percentage || ''} disabled />
                  </div>
                  ...
                  <div className="col-span-2">
                    ...
                    <textarea 
                      className="textarea text-slate-100 dark:text-gray-700" 
                      value={descriptionEn}
                  ...
                  <div className="col-span-2">
                    <label className="form-label mb-1">Branch Name In English</label>
                    <input
                      className="input"
                      value={branchNameEn}
                  ...
                  <div className="col-span-2">
                    <label className="form-label mb-1">Branch Name In Arabic</label>
                    <input
                      className="input"
                      value={branchNameAr}
              ...
              <div className="card p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input className="input" value={provider.user?.name || ''}  />
                  </div>
                  <div>
                    <label className="form-label mb-1"> Email</label>
                    <input className="input" value={provider.user?.email || ''}  />
                  </div>
                  <div>
                    ...
                    <MuiPhoneInput
                      ...
                      onChange={(value, info) => {
                        ...
                        const cc = info?.countryCallingCode || userCountryCode || '20'; // fallback to Egypt
