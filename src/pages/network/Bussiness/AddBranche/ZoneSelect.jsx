...
const ZoneSelect = ({ formik }) => {
    ...
    useEffect(() => {
      const fetchZones = async () => {
        ...
        try {
          ...
          const response = await axios.get(`${import.meta.env.VITE_APP_API_URL}/zones/list?filter[city_uid]=${formik.values.city_uid}`, {
            headers: {
    ...
    const handleChange = (selectedOption) => {
      formik.setFieldValue('zone_uid', selectedOption ? selectedOption.value : '');
      formik.setFieldValue('zone_name', selectedOption ? selectedOption.label : '');
    };
    ...
    return (
      <div className="flex flex-col gap-1">
        ...
        <Select
          ...
          isLoading={loading}
          options={options}
          value={options.find(opt => opt.value === +formik.values.zone_uid) || null}
          onChange={handleChange}
          placeholder={formik.values.city_uid ? "Select Zone" : "Please select a City first"}
          styles={customStyles}
          isDisabled={!formik.values.city_uid} // 🔒 Disable if no city selected
          onBlur={() => formik.setFieldTouched('zone_uid', true)}
        />
