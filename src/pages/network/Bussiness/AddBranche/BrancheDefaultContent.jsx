...
const BrancheDefaultContent = ({ provider }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      try {
        ...
        const apiError = error.response?.data?.message || 'Error creating branch';
  ...
  return (
    ...
    <div className="col-span-1">
        ...
        <input className="input" value={formik.values.name} onChange={(e) => formik.setFieldValue('name', e.target.value)} onBlur={() => formik.setFieldTouched('name', true)} />
    ...
    <p className="text-red-500 text-xs mt-1">{formik.errors.name}</p>
      ...
      <div className="col-span-1">
        ...
        <input className="input" value={formik.values.name_ar} onChange={(e) => formik.setFieldValue('name_ar', e.target.value)} onBlur={() => formik.setFieldTouched('name_ar', true)} />
        {formik.touched.name_ar && formik.errors.name_ar && (
    <p className="text-red-500 text-xs mt-1">{formik.errors.name_ar}</p>
  )}
      </div>
  ...
  {formik.touched.status && formik.errors.status && (
    <p className="text-red-500 text-xs mt-1">{formik.errors.status}</p>
      ...
      <div className="col-span-2">
        ...
        <input className="input" value={formik.values.address}
    ...
    <p className="text-red-500 text-xs mt-1">{formik.errors.address}</p>
      ...
      <div className="col-span-2">
        ...
        <input className="input" value={formik.values.address_ar}
        onChange={(e) => formik.setFieldValue('address_ar', e.target.value)}
        onBlur={() => formik.setFieldTouched('address_ar', true)} />
        {formik.touched.address_ar && formik.errors.address_ar && (
    <p className="text-red-500 text-xs mt-1">{formik.errors.address_ar}</p>
  )}
      </div>
      ...
      <div className="flex items-center justify-between gap-2 mt-4 col-span-2 px-4 dark:bg-gray-100 border border-gray-300 p-4 rounded-md">
        ...
        <input
          ...
          onChange={(e) => formik.setFieldValue('main_branch', e.target.checked ? 1 : 0)}
      ...
      <div>
        ...
        <input className="input" value={formik.values.latitude} onChange={(e) => formik.setFieldValue('latitude', e.target.value)} onBlur={() => formik.setFieldTouched('latitude', true)} />
    ...
    <p className="text-red-500 text-xs mt-1">{formik.errors.latitude}</p>
      ...
      <div>
        ...
        <input className="input" value={formik.values.longitude} onChange={(e) => formik.setFieldValue('longitude', e.target.value)} onBlur={() => formik.setFieldTouched('longitude', true)} />
    ...
    <p className="text-red-500 text-xs mt-1">{formik.errors.longitude}</p>
      ...
      <div className="col-span-2">
        <MapContainer
          center={[
            formik.values.latitude ? parseFloat(formik.values.latitude) : 28.5,
            formik.values.longitude ? parseFloat(formik.values.longitude) : 34.5,
          ]}
          zoom={13}
        ...
        />
      <LocationSelector setLatLng={({ latitude, longitude }) => {
