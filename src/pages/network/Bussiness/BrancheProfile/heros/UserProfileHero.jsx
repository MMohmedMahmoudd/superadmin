...
const UserProfileHero = ({
  ...
  name,
  info,
  status,            // ✅ receive the status
  onStatusChange     // ✅ receive status change handler
}) => {
  const {
    getThemeMode
  } = useSettings();
    ...
    return <div className="bg-center bg-cover bg-no-repeat hero-bg" style={{
      ...
      />
            ...
            <select
                ...
                className={`select  w-auto  btn-outline ${status === 'active' ? 'btn-success' : status === 'inactive' ? 'btn-danger' : 'btn-warning '}`}
              >
