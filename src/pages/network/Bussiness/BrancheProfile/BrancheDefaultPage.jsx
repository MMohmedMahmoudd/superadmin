...
const BrancheDefaultPage = () => {
  const { id } = useParams();
  const branchId = new URLSearchParams(window.location.search).get('branch');
  const [provider, setProvider] = useState(null);
  const [error, setError] = useState(null);
  const [branch, setBranch] = useState(null); // store specific branch
  ...
  useEffect(() => {
    const fetchProvider = async () => {
      try {
        ...
        if (res.data?.data) {
          const fetchedProvider = res.data.data;
          const allBranches = [
            ...(fetchedProvider.branches || []),
            fetchedProvider.main_branch
          ];
  ...
  onStatusChange={async (newStatus) => {
    try {
      ...
      enqueueSnackbar('Provider status updated successfully!', { variant: 'success' });
      ...
      enqueueSnackbar('Failed to update provider status.', { variant: 'error' });
    }
  }}
      />
      <Container>
      <BrancheDefaultContent branchId={branchId} branch={branch} provider={provider} />
      </Container>
    </Fragment>
};

export { BrancheDefaultPage };
