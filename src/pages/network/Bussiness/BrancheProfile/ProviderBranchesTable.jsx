...
import { CitySelect, ZoneSelect } from './';
...
const ProviderBranchesTable = ({ providerId }) => {
    ...
    try {
      ...
      const filteredBranches = branchesData.filter(branch => {
        ...
        if (
          ...
          branch.city?.name?.toString().toLowerCase() !== formik.values.city_name.toLowerCase()
        ) {
          return false;
        }
        ...
        // Handle zone filter 
        if (
          formik.values.zone_name &&
          branch.zone?.name?.toString().toLowerCase() !== formik.values.zone_name.toLowerCase()
        ) {
          return false;
        }
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      accessorKey: 'address',
      cell: ({ row }) => (
        <div className="text-sm whitespace-pre-line">{row.original.address}</div>
      ),
    ...
    },
    {
      ...
      header: 'Status',
      cell: ({ row }) => (
        <span className={`badge badge-${row.original.status === 1 ? 'success' : row.original.status === 0 ? 'danger' : 'warning'} badge-outline capitalize`}>
          ● {row.original.status === 1 ? 'Active' : row.original.status === 0 ? 'Inactive' : 'Waiting Confirmation'}
        </span>
      ),
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <button
          ...
          onClick={() => navigate(`/addBranch/${row.original.id}`)}
        >
    ...
    <div className="card-body p-0 overflow-x-auto relative">
      <div className="card-header px-2 flex-wrap gap-2 justify-between">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <select className="select select-sm select-bordered w-24 h-9 "
            value={status}
            onChange={e => {
          ...
          <button className="btn btn-sm btn-outline btn-primary" onClick={() => navigate(`/network/add-branch`)}>Add New Branch</button>
        </div>
      </div>
      ...
      {loading && (
        <div className="absolute inset-0 bg-black/5 backdrop-blur-[1px] flex items-center justify-center">
          <div className="flex items-center gap-2 px-4 py-2 dark:bg-black/50 bg-white/90 rounded-lg shadow-lg">
            ...
            <span className="text-sm font-medium text-gray-700">Loading branches...</span>
          </div>
        </div>
      )}
